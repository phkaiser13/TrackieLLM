...
GPU_BACKEND(`Metal (Apple Silicon)`, `The backend for Apple's M-series chips, using the Metal API for low-level GPU programming. It is optimized for the unified memory architecture of Apple Silicon.`, `tk_metal_dispatch.mm`, `Handles dispatching of Metal compute shaders.`, `tk_metal_kernels.metal`, `Contains the Metal compute shaders.`, `tk_metal_helpers.mm`, `Provides helper functions for working with Metal.`)
